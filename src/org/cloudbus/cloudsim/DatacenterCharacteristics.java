...
public class DatacenterCharacteristics {
	...
	public int getMipsOfOnePe() {
		if (getHostList().size() == 0) {
			return -1;
                ...
                PEs' MIPS.*/
		return (int) PeList.getMips(getHostList().get(0).getPeList(), 0);
	}
	...
	public int getMipsOfOnePe(int id, int peId) {
		...
		return (int) PeList.getMips(HostList.getById(getHostList(), id).getPeList(), peId);
	}
	...
	public double getMips() {
		...
		switch (getAllocationPolicy()) {
			...
			case DatacenterCharacteristics.OTHER_POLICY_SAME_RATING:
				mips = (long)getMipsOfOnePe() * HostList.getNumberOfPes(getHostList());
			break;
