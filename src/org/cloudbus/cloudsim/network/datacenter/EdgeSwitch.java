...
public class EdgeSwitch extends Switch {
     ...
     */
	private List<AggregateSwitch> uplinkswitches;
	...
	public EdgeSwitch(String name, int level, NetworkDatacenter dc) {
		...
		setUplinkswitches(new ArrayList<AggregateSwitch>());
	}
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.EDGE_LEVEL) {
			...
			AggregateSwitch sw = getUplinkswitches().get(0);
	...
	public List<AggregateSwitch> getUplinkswitches() {
		return uplinkswitches;
	}

	public void setUplinkswitches(List<AggregateSwitch> uplinkswitches) {
