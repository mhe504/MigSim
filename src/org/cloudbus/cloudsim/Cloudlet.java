...
public class Cloudlet {
    ...
    public Cloudlet(
        ...
        execStartTime = 0.0;
        setFinishTime(-1.0);    // meaning this Cloudlet hasn't finished yet
        classType = 0;
        netToS = 0;
    ...
    public void setCloudletStatus(final int newStatus) throws Exception {
        ...
        if (newStatus == Cloudlet.SUCCESS) {
        	setFinishTime(CloudSim.clock() * calcVariationFactor());
        }
    ...
    private double calcVariationFactor() {
		
    	Random r = new Random();
    	BigDecimal randomValue = new BigDecimal( 0.00 + (1.00 - 0.00) * r.nextDouble());
    	BigDecimal roundedVal = randomValue.setScale(2, BigDecimal.ROUND_HALF_EVEN);
    	roundedVal = roundedVal.divide(new BigDecimal("100"));
    	
    	if (r.nextBoolean())
    		roundedVal = roundedVal.add(BigDecimal.ONE);
    	else
    		roundedVal = BigDecimal.ONE.subtract(roundedVal);
    ...
    private void setFinishTime (double t)
    {
    	finishTime = t;
    }
